...
> 📖 来源：数据搬砖日记《大模型能自动构建本体吗：AI 可发现候选但不能替业务定义现实》（微信公众号，2026-09-16）；老李说数《企业本体落地系列④：业务部门深度参与——三类角色四环节与标签争议裁决》（微信公众号，2026-09-18）；虎嗅《企业该买模型还是养上下文：专有模型四件套拆解》（微信公众号，2026-09-16）——三源跨域互证（公众号原文无公开 URL，存档见知识库同名笔记）技能（编程、写作）可以依托公共模型，但定义企业独特性的专属智能必须完全掌握在自己手中。「我无法想象当我需要提升企业自身智能水平的时候，还要去向第三方求助——我必须在企业内部直接完成这件事。」 sofagent 的 MIT 开源 + 本地部署 + 数据不出本机，正是这一原则的工程落地。
