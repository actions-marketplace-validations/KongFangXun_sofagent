...
# 1f. 归并配额棘轮（公约第 10 行「归并配额（硬门槛）」的机器判据 · 维度数只降不升）
#     公约原文：新增 N 维 → 本版必须先真实归并 ≥N 维（被并内容实际移入目标维度，
#     git diff 可查；注释压缩不算）；有同类 → 扩展旧维度子项、不新增编号。
#     ⇒ 合法演进下维度计数只可能持平或下降，永不增长。
#     实案：d805d8d9 净增 9 行判据却未对销归并——当时仅靠行数代理（1953 > 1950）才暴露，
#     配额本身无人检查。本段即补该缺口。
#     基线来源：8b28942d 两次真实归并后实测 85（grep -c "^#### " 口径）——
#     基线只应被后续归并**下调**，不因新增而上调。
DIM_BASELINE=85
if [ "${ACTUAL_DIM}" -le "${DIM_BASELINE}" ]; then
  ok "维度计数 ${ACTUAL_DIM} ≤ 归并配额基线 ${DIM_BASELINE}（棘轮：只降不升）"
else
